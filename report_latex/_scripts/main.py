...
# Initialize Attack Engine (nap du lieu mot lan khi khoi dong)
attack_engine = AttackEngine()


@app.get("/stats")
async def get_stats():
    """Tra ve thong ke tap du lieu: do thua va phan phoi diem."""
    try:
...
async def search_movies(q: str = Query(..., min_length=1)):
    """Tim phim theo ten, kem support va trong so wt(i) cua tung phim."""
    try:
...
async def run_attack(
    ...
    """Thuc thi tan cong giai an danh dua tren tri thuc bo tro da biet."""
    try:
        ...
        for item in aux_info:
            ...
            if "date" in item and item["date"]:
                ...
                rec["timestamp"] = 883612800  # mac dinh 1998-01-01
...
@app.get("/auto_aux")
async def auto_aux(
    user_id: int = Query(None, description="ID nan nhan; bo trong = chon ngau nhien"),
    num_movies: int = 3,
    rating_error: int = 0,
    ...
    """
    Sinh "tri thuc bo tro" tu mot nguoi dung CO THAT, cho phep them nhieu de
    minh hoa tinh ben vung. Tra ve danh sach phim kem ten de nap thang vao
    bang tri thuc cua ke tan cong.
    """
    try:
        engine = get_db_engine()
        if user_id is None:
